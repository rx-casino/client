...
const appData = {
  currencyName: "SOL",
  currencyImage:
    "https://cryptologos.cc/logos/thumbs/solana.png?v=034",
...
export default class WalletManager extends EventEmitter {
  ...
  constructor() {
    ...
    const updateWallet = (wallet) => {
      if (wallet && wallet.coin_name) {
        let minAmount = wallet.coin_name === "Fun Coupons" ? 0.01 : wallet.coin_name === "SOL" ? 0.01 : 1;
        let maxAmount = wallet.coin_name === "Fun Coupons" ? 100_000 : wallet.coin_name === "SOL" ? 5_000 : 30_000;
        const usdPrice = wallet.coin_name === "Fun Coupons" ? 0 : (wallet.coin_name === "SOL" ? 1 : 0.1);
  ...
  async syncData() {
    const [fun_coupon, sol] = await Promise.all([
      fetchWallet("fun"),
      fetchWallet("sol"),
    ]);

    const balances = [fun_coupon, sol].map(
      (wallet) => ({
        amount: wallet.balance,
        currencyName: wallet.coin_name,
        currencyImage: wallet.coin_image,
    ...
    runInAction(() => {
      ...
      balances
        ...
        .forEach((balance) => {
          const minAmount = balance.currencyName === "Fun Coupons" ? 0.01 : balance.currencyName === "SOL" ? 0.1 : 1;
          const maxAmount = balance.currencyName === "Fun Coupons" ? 100_000 : balance.currencyName === "SOL" ? 5_000 : 30_000;
          const usdPrice = balance.currencyName === "Fun Coupons" ? 0 : (balance.currencyName === "SOL" ? 1 : 0.1);
  ...
  amountToFiat(amount, token = this.current.currencyName, fiat = this.preferredFiatCurrency, rates = this.currencyRates) {
    ...
    const usdPrice = token === "Fun Coupons" ? 0 : token === "SOL" ? 0.1 : 1; //this.dict[currency].usdPrice
